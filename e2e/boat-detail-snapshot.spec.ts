...
const calendarFor = (ym: string, priceLine?: string) => ({
  ok: true,
  meta: {
    uid: '9999',
    stUid: '1',
  ...
  ym,
  days: priceLine
    ? [{ date: `${ym.slice(0, 4)}-${ym.slice(4)}-15`, day: 15, tide: '', status: 'available', priceLine }]
    : [],
});
...
test('달력 월만 넘기면 가격 변동으로 오판하지 않는다', async ({ page }) => {
  ...
  await page.route('**/api/boat-calendar*', (route) => {
    ...
    const ym = new URL(route.request().url()).searchParams.get('ym')!;
    return route.fulfill({ json: calendarFor(ym, priceLine) });
  });
  ...
  await expect(page.getByRole('region', { name: '예약 달력' })).toHaveAttribute('aria-busy', 'false');
  await expect(banner).toContainText('90,000');
  await expect(banner).toContainText('110,000');
