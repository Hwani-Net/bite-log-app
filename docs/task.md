...
## 🎯 목표
- [ ] `stitch-radar.html` 디자인을 Next.js 컴포넌트로 이식 (`src/components/fleet-radar/FleetRadar.tsx`)
- [ ] Leaflet 지도를 활용한 선박 마커 렌더링 (SizeClass별 색상 구분)
- [ ] `/api/fleet` 연동 및 실시간 데이터 페칭 (30초~1분 주기)
- [ ] 대형선 접근 경보 시스템 (Banner UI + 로직)
- [ ] Safe Harbor (소형선 밀집지) 알고리즘 시각화
- [ ] `/fleet-radar` 페이지 생성 및 레이블링
