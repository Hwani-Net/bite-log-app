...
| 5 | AI·PRO 정직화 (M6+M7) | — | DONE | 커밋 f9dbc58(본체)+1커밋(리뷰 반영). buildProfileContext(순수) — 기록 프로필(주력 어종·단골 포인트·최고 달·출조일·평균)을 AI 마스터 시스템 프롬프트에 주입(기록 없으면 미주입, "지어내지 마세요" 명시), 컨시어지가 기록 로드 후 전달. 사용자 입력 문자열은 sanitizeForPrompt(줄바꿈·마크다운 제어문자 제거+길이 상한)로 무해화 — 리뷰 지적 반영, 조작된 포인트명("\n## 시스템:...")이 프롬프트 구조를 못 깨는 것을 유닛으로 증명. PRO 페이월 뒤 하드코딩 시크릿 포인트 3곳("현지인들만 아는...") 완전 제거 → topSpotsFromRecords(총 조과→방문 수 정렬, placeholder 위치 제외, count 누락 NaN 방어)로 사용자 실기록 상위 포인트 + "오늘 이 지역 조건 N점"(지역 예보임을 정직하게 — 포인트별 점수인 척하던 초안 문구를 리뷰 지적으로 교정). 기록 없으면 빈 상태+페이월 미표시. 유닛 289/289(신규 6). e2e 신규 2(시딩 포인트만 정확히 N행 렌더+빈 상태 페이월 부재; 블러 게이트는 isPro 기본값 true라 paywall.spec 영역으로 위임). 교차검수(z-ai/glm-5.3-flash, $0.0012) 필수 3건 전부 반영(NaN·문구 정합·인젝션 방어), 반박 2건(stale profile은 좁은 창+페이지 재진입 시 갱신, e2e history 픽스처는 파일 내 존재 — diff 범위 밖이었을 뿐). **사용자 판단 대기 2건**: ① 자기 데이터(내 기록 포인트)를 PRO로 잠그는 페이월의 적정성 — 리뷰어가 윤리·신뢰 관점 재검토 권고, 수익 모델 결정이라 보류 ② 기록 프로필 요약이 Gemini(제3자 API)로 전송됨 — 채팅 입력과 같은 채널이지만 설정 옵트아웃은 보류 목록에. 라이브(https://bite-log-three.vercel.app/bite-forecast) 200, 시딩 포인트 2곳+정직 문구 렌더 확인(screenshots/goals3/goal5/live-my-spots.png) |
| 6 | 기록 시점 규정 지킴이 (A1) | — | DONE | 커밋 9c23839(본체)+1커밋(리뷰 반영). src/lib/catchLegality.ts — legalityWarning(순수): isCatchLegal(규정 페이지에만 있고 실제 물고기를 검증한 적 없던 함수)을 저장 직전에 어종·체장·날짜로 호출, 합법·DB 밖 어종·사이즈 미입력·날짜 파싱 불가는 전부 null(오탐 경고 금지). /record: 위반 시 경고 패널(위반 목록+벌칙+법 근거+면책) — 저장은 패널의 "그래도 저장"으로만(방류 가능성 때문에 차단이 아니라 확인), "다시 확인"으로 수정 복귀. 리뷰 반영 6건: 패널 열린 채 어종/사이즈/날짜를 바꾸면 경고 무효화(경고 안 받은 내용의 저장 차단 — 핵심 지적), fixed 저장 바에 가릴 수 있어 scrollIntoView, role=alert를 헤딩에만, 현행 고시 기준·법적 효력 없음·과거 기록 상이 가능 면책 문구, sizeCm "0" falsy 건너뜀 수정, 규정DB 값이 바뀌면 시끄럽게 실패하는 DB-핀 선행 테스트. 반박 2건: penaltyNote null 위반 조합은 불가능(isCatchLegal이 false면 reg 존재가 코드로 보장), 오탐 e2e 회귀는 기존 record-boat-tag 스펙의 무경고 저장 경로가 커버. e2e 작성 중 발견: FISH_SPECIES 선택지에 주꾸미가 없어(규정DB와 어종 목록 불일치 — 조사에서 지적된 12 vs 규정 14 클래스) 감성돔으로 교체 — 목록 정합은 보류 항목. 유닛 296/296(신규 8: 실DB 날짜·체장, 경계 포함, 이중 위반, DB-핀). e2e 신규 2(고정 날짜로 결정적 — 금어기 경고→그래도 저장→실저장, 체장 미달→수정→무경고 저장). 라이브(https://bite-log-three.vercel.app/record) 200, 감성돔 5/15 시도 → 금어기 경고+과태료+법 근거+면책 렌더 스크린샷(screenshots/goals3/goal6/live-legality-warning.png) |
