...
## 라이브 결함 대응 — /booking "Application error" (2026-08-28)

사용자가 프로덕션 `/booking` 에서 Next.js 기본 오류 화면을 봤다고 보고.
헤드리스 검증은 계속 통과하고 있었으므로 **검증 방식이 틀린 것**으로 보고
재현부터 했다.

**재현 시도와 결과** — 넷 다 재현 실패(정상 렌더):
1. 새 프로필 — 정상
2. 실사용에 가까운 localStorage(기록 12건·내 배·관심목록) 주입 — 정상
3. 캐시에 옛 HTML(죽은 청크 참조)을 심어 재방문 — **크래시 아님**, 하이드
   레이션만 안 됨. 오류 화면은 React가 떠서 던져야 나온다.
4. 실제 재배포를 끼운 교차 배포 재방문 — 정상(옛 청크가 아직 200)

**확인된 사실**: 앱에 `error.tsx`·`global-error.tsx` 가 **하나도 없었다**.
그래서 어떤 클라이언트 예외든 복구 수단도 원인 표시도 없는 Next 기본
화면으로 직행했다. 원인 규명과 별개로 이게 사용자가 갇힌 이유다.

**조치**
- `public/sw.js`: 문서 navigate 를 network-first 로(옛 HTML 배포 금지),
  캐시 v5. — 근거: 지난 배포의 청크 경로는 404, 현재 경로는 200 실측.
- `src/app/global-error.tsx`: 한국어 복구 화면(다시 시도 / 캐시·SW 비우고
  새로고침 / 원인 문자열).
- `src/app/layout.tsx`: React 이전에 죽는 청크 오류용 인라인 자가복구
  (세션당 1회 새로고침).

**남은 것**: 근본 원인 미확정. 다음 발생 시 사용자가 오류 화면의 문자열을
그대로 읽어 주면 그때 확정한다 — 그게 복구 화면에 원인을 띄운 이유다.
