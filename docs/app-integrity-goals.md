...
| 5 | 죽은 코드·거짓 문구 대청소 (I5) | 2 | DONE | 커밋 2개(본체+리뷰 반영). 0-importer 서비스 5개+고아 API 라우트 삭제(~900줄, grep 재검증 — ParsedNotice 타입은 openRunAlertService 자체 정의라 파손 없음). /alerts "어떻게 작동하나요?"의 거짓 문구(실행된 적 없는 공지 파서 설명) → 실동작(구독 저장·금어기 해제 알림·포그라운드 한정·공지 연동은 준비 중)으로 ko/en 재작성. 미태그 하드코딩에 @mock-data 태그(컨시어지 지식베이스 4개·쿠팡 링크 — 링크 부패 주기 점검 메모 포함). **조용한 가짜 물때 정직화**: getMockTideData가 4개 실패 경로에서 무표시 반환하던 것에 TideData.mocked 플래그 — bite-forecast·기록 폼에 "예시" 배지(뉴스 페이지 관행), **기록 저장 시 mock 물때 제외**(지어낸 물때가 DNA·조건표를 오염 못 함). 교차검수(z-ai/glm-5.3-flash, $0.0009) 반영 3건: 과거 오염 기록은 "(예측)" 관측소명으로 식별해 DNA·조건표에서 제외(+유닛), e2e 날씨 고정(무관한 외부 실패 차단), TideData 생성처 전수 확인(mock·실측 2곳뿐 — grep 실증). 반박 2건(route 차단 무효 의심 — tideService가 로컬 /api/tide 프록시 경유임을 소스로 확인했고 배지 발현 자체가 폴백 실행의 증명, NEXT_PUBLIC_KHOA 노출 의심 — tide는 서버 라우트가 키 보유). 유닛 322/322. e2e 신규 2(/alerts 문구 정직성, KHOA 차단→예시 배지+저장 기록에 tide 부재+날씨는 정상 저장). 라이브 200 — 공지 파서 문구 부재+금어기 해제 언급 확인, /api/fishing-index 404(제거 확인)(screenshots/goals4/goal5/live-alerts-honest-copy.png) |
| 6 | PWA 정합 (I6) | — | DONE | 커밋 2개(본체+리뷰 반영). manifest theme/background를 옛 라이트 팔레트(#1392ec/#f6f7f8)에서 실제 다크(#080d14)로 — 설치 스플래시 흰 번쩍임 제거. sw v4 — 프리캐시에 누락 라우트 5개(booking/trip-plan/catch-value/fishdex/alerts) 추가, install을 addAll(하나 실패=업데이트 전체 차단, 약전계 낚시 환경에 치명)에서 페이지별 allSettled 격리로(리뷰 반영). A2HS — beforeinstallprompt를 **모듈 평가 시점**에 캡처(hydration 이전 발화 유실 레이스 차단 — 리뷰 반영)해 설정의 "홈 화면에 추가" 버튼으로, 이벤트 없으면(iOS·설치됨·미지원) 버튼 자체가 없음, 소진 선행으로 연타 이중 prompt() 차단, appinstalled 처리. 교차검수(z-ai/glm-5.3-flash, $0.0008, Request changes) 반영 4건(실패 격리·모듈 시점 캡처·소진 선행·행위 e2e — 버전 문자열 리터럴 검사 대신 caches.keys() 실측+조기 발화 레이스 회귀), 반박 2건(구캐시 삭제·clients.claim은 activate에 이미 존재 — 소스 실증, layout meta theme-color는 이미 #080d14). e2e 신규 5(manifest 색, sw 목록, v4 캐시 실형성+v3 부재, 조기 발화 캡처, 버튼 라이프사이클+prompt 호출+소진). 라이브 200 — manifest 색·sw 라우트 5개·**브라우저에서 bitelog-v4 캐시 실형성(v3 없음)**·설치 버튼 노출 확인(screenshots/goals4/goal6/live-a2hs.png) |
