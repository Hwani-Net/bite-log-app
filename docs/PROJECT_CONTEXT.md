...
| 9 함대 레이더 | 실시간 선박 밀집도 히트맵 | 🔵 구현 중 (백엔드 완료) |
...
### Phase 9: 실시간 함대 레이더 (Fleet Radar) 🔵 진행 중
- [x] **백엔드 프록시 매퍼 (Mapper)** — AIS 동적 위치 + 정적 톤수 데이터 MMSI 조인 API (`/api/fleet`) 구현 완료 (2026-03-15)
- [ ] **레이더 UI 구현** — `stitch-radar.html` 기반 Heatmap & Tonnage Marker 시각화
- [ ] **안전 구역 알고리즘** — 대형선 반경 500m 회피 소형선 밀집지 추출 로직 적용
- [ ] **접근 알림** — 사용자 GPS 기반 대형선 근접 푸시 알림
- [ ] **PRO 기능 고도화: 시크릿 포인트 2.0+** — 실시간 조황(Free) + **함대 밀집도 레이더** + 수중 지형도(KIGAM) 연합. 내 배가 '죽은 포인트'에 있는지 '집단 학살지'에 있는지 즉시 판독.
