    ...
    <div className="flex gap-0.5 items-end">
      {months.map((m) => (
        <div key={m} className="flex flex-col items-center gap-0.5 flex-1">
          ...
          <div
            className={`w-full h-2 rounded-sm ${getMonthColor(m)} ${m === now ? "ring-1 ring-blue-500 ring-offset-1" : ""}`}
          />
          <span
            className={`text-[8px] ${m === now ? "text-blue-600 font-bold" : "text-slate-400"}`}
          >
            {m}
          </span>
        </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-[9px] px-2 py-0.5 rounded-full text-white font-bold ${config.badge}`}
            >
              {config.label}
            </span>
          </div>
...
export default function SeasonForecastPage() {
  ...
  return (
    <main className="mx-auto max-w-md min-h-screen bg-gradient-to-b from-sky-50 to-white dark:from-bg-dark dark:to-bg-dark pb-24">
      {/* Header */}
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-bg-dark/90 backdrop-blur-lg border-b border-slate-100 dark:border-slate-700/50">
