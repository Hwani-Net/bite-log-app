...
export default function AIConciergeCard({ locale }: AIConciergeCardProps) {
  ...
  const inSeason = useMemo(
    () =>
      sortByCurrentSeason(FISH_SEASON_DB, month)
        .filter((d) => {
            ...
            <div className="flex flex-wrap gap-1.5">
              {inSeason.map((d) => {
                ...
                return (
                  <span
                    key={d.species}
                    className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold ${isGold ? "bg-amber-400/30 text-amber-100" : "bg-white/15 text-white/90"}`}
                  >
                    <span
