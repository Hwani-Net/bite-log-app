  ...
  return (
    <section className="px-4 pt-4">
      ...
      <div className="min-h-[200px]">
        ...
        {activeTab === "news" && <NewsTab news={topNews} locale={locale} />}
        {activeTab === "forecast" && (
    ...
    return (
      <div className="space-y-2.5">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="bg-white dark:bg-slate-800/50 rounded-2xl p-3 flex gap-3 animate-pulse border border-slate-100 dark:border-slate-700/50"
          >
  ...
  return (
    <div className="space-y-2.5">
      {records.slice(0, 3).map((r, i) => {
        ...
        const hasPhoto =
          r.photos.length > 0 && !r.photos[0].startsWith("/fish-");
        return (
          <Link
            key={r.id}
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <div className="flex items-center justify-between gap-2">
                <span
                  className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${tagColors[i % 3]}`}
                >
                  {r.species}
                </span>
      ...
      <Link
        href="/records"
        className="flex items-center justify-center gap-1 text-xs font-semibold text-primary py-2 hover:bg-primary/5 rounded-xl transition-colors"
      >
          ...
          >
            <div className="relative w-full h-36 overflow-hidden">
              ...
              <div className="absolute top-2 left-2.5 flex items-center gap-1.5">
                ...
                />
                <span className="text-[9px] font-bold text-white/80 uppercase bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded-full">
                  {item.sourceLabel}
                </span>
                {item.species && (
...
function ForecastTab({
  ...
  const isKo = locale === "ko";
  const scores: SpeciesBiteScore[] = biteTime
    ? getSpeciesBiteScores(biteTime)
    : [];
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${colors} transition-all duration-500`}
                  style={{ width: `${s.score}%` }}
                />
              </div>
