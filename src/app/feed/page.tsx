      ...
      <main className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        {loading && (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white border border-slate-100 shadow-sm rounded-2xl p-4 shadow-sm animate-pulse">
        ...
        {filteredFeed.map((item) => {
          ...
          return (
            <article key={item.id} className="bg-white border border-slate-100 shadow-sm rounded-2xl shadow-sm overflow-hidden">
              {/* User header */}
              <div className="px-4 pt-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-800 truncate">{item.userDisplayName}</p>
                ...
                <div className="px-4 pb-3 space-y-2">
                  {item.comments?.map((c) => (
                    <div key={c.id} className="flex items-start gap-2">
                      ...
                      <div className="flex-1 bg-slate-50 rounded-xl px-3 py-2">
                        <p className="text-[10px] font-semibold text-slate-600">{c.userDisplayName}</p>
                  ...
                  {showComments && (
                    <div className="flex items-center gap-2 mt-1">
                      <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleComment(item)} placeholder={locale === 'ko' ? '댓글을 입력하세요...' : 'Add a comment...'} className="flex-1 bg-slate-100 rounded-full px-4 py-2 text-xs text-slate-900 outline-none focus:ring-2 focus:ring-primary/30" />
