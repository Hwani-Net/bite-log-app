...
function RecordDetailContent() {
  ...
  const inputCls =
    "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:border-primary focus:ring-1 focus:ring-primary transition-all text-slate-900";

  if (!record) {
    return (
      <div className="page-enter relative z-10 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <Fish size={48} className="text-slate-300 animate-pulse" />
  ...
  return (
    <div className="page-enter relative z-10 pb-24">
      {/* Header */}
      <header className="sticky top-0 z-50 flex items-center bg-white/80 dark:bg-bg-dark/90 backdrop-blur-md px-4 py-4 justify-between border-b border-slate-200/50 dark:border-slate-700/50">
        <button
          onClick={() => router.back()}
          className="text-slate-900 flex size-10 shrink-0 items-center justify-center hover:bg-slate-100:bg-slate-800 rounded-full transition-colors"
        >
          <ArrowLeft size={20} />
        </button>
          ...
          {editing ? (
            ...
            <>
              ...
              <button
                ...
                className="text-red-400 size-10 flex items-center justify-center rounded-full hover:bg-red-50:bg-red-900/20 transition-colors"
              >
                <Trash2 size={20} />
              </button>
              <button
      ...
      {editing ? (
        ...
        <div className="px-4 pt-4 space-y-4">
          <div className="bg-white dark:bg-surface-dark border border-slate-100 dark:border-slate-700/50 shadow-sm rounded-2xl p-4 shadow-sm">
              ...
              <label className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between bg-slate-50 rounded-xl p-1 border border-slate-200">
                  <button
                    ...
                    className="size-9 flex items-center justify-center bg-white rounded-lg shadow-sm text-slate-600"
                  >
                  ...
                  <button
                    ...
                    className="size-9 flex items-center justify-center bg-primary rounded-lg shadow-sm text-white"
                  >
        ...
        <div className="px-4 pt-6 space-y-4">
          ...
          {/* Tide info */}
          {record.tide && (
            <div className="bg-white dark:bg-surface-dark border border-slate-100 dark:border-slate-700/50 shadow-sm rounded-2xl p-4 shadow-sm bg-blue-50/50">
              <div className="flex items-center gap-2 text-slate-500 text-xs mb-3">
                <DynamicIcon name="waves" size={14} className="text-primary" />
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                {record.tide.tides.map((t, i) => (
                  <span
                    key={i}
                    className={`flex flex-col items-center justify-center p-2 rounded-xl flex-1 min-w-[60px] ${t.type === "High" ? "bg-blue-100 text-blue-600" : "bg-cyan-100 text-cyan-600"}`}
                  >
      ...
      {shareToast && (
        <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-slate-900 text-white text-sm font-semibold rounded-full shadow-xl animate-pulse">
          <CheckCircle size={14} className="inline align-middle mr-1" />
          {shareToast}
        </div>
      )}
    </div>
