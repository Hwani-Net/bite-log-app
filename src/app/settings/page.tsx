    ...
    <div className="page-enter relative z-10 px-4 pt-6 pb-24">
        ...
        <div className="glass-card rounded-2xl p-5 shadow-sm">
          {isLoggedIn && user ? (
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={signOut}
                className="px-3 py-1.5 rounded-full bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 text-xs font-medium transition-colors hover:bg-red-100 dark:hover:bg-red-900/40"
              >
            ...
            <div className="flex flex-col items-center gap-4 py-2">
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center gap-3 px-6 py-3 rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 shadow-sm hover:shadow-md transition-all active:scale-95 disabled:opacity-50"
              >
            ...
            {themes.map(({ value, icon, label, desc }) => (
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  theme === value ? 'bg-gradient-to-tr from-primary to-cyan-400 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'
                }`}>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
          {[
            ...
            <div key={item.key}>
              ...
              {i < 2 && <div className="h-[1px] bg-slate-100 dark:bg-slate-700" />}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link href="/booking" className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center gap-2 hover:border-primary/30 transition-colors">
            <span className="text-2xl">🎣</span>
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{locale === 'ko' ? '낚시 예약' : 'Booking'}</span>
          </Link>
          <Link href="/news" className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center gap-2 hover:border-primary/30 transition-colors">
            <span className="text-2xl">🔥</span>
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{locale === 'ko' ? '조과 소식' : 'News'}</span>
          </Link>
        </div>
      </section>
      ...
      <section className="mb-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
          <div className="flex items-center justify-between py-2">
            ...
            <span className="text-sm font-medium text-slate-900 dark:text-slate-100">1.1.0</span>
          </div>
