...
export function filterByRegion(
  ...
  return data.releaseSites.filter(
    (s) => s.region === region || s.region === "전국",
  );
}
...
export function sortByCurrentSeason(
  species: FishSeasonData[],
  month: number,
): FishSeasonData[] {
...
export const SPECIES_META: Record<string, { speciesEn: string }> =
  Object.fromEntries(
    FISH_SEASON_DB.map((d) => [d.species, { speciesEn: d.speciesEn }]),
  );
