...
export const FISH_REGULATION_DB: FishRegulation[] = [
    ...
    tipKo: "15cm 미만은 즉시 방류. 작은 볼락이 많이 올라오면 포인트 이동 추천." },
  {
    ...
    minSizeCm: 21,
    dailyLimit: null,
    regionalNotes: ["전 해역 포획금지 체장 21cm 미만"],
    penaltyNote: "체장 미달 포획 시 100만원 이하 과태료",
    legalRef: "수산자원관리법 시행규칙 별표4",
    category: "saltwater",
    tipKo:
    ...
    tipKo: "30cm 미만 즉시 방류. 겨울 방어가 가장 맛있음 (11~1월)." },
  {
    ...
    minSizeCm: 15,
    dailyLimit: null,
    regionalNotes: ["전 해역 포획금지 체장 15cm 미만"],
    penaltyNote: "체장 미달 포획 시 100만원 이하 과태료",
    legalRef: "수산자원관리법 시행규칙 별표4",
    category: "saltwater",
    ...
    tipKo: "겨울 대구 낚시 시 금어기 주의. 특히 진해만 해역은 별도 규정." },
  {
    ...
    minSizeCm: null,
    dailyLimit: null,
    regionalNotes: ["서해·남해 전역 적용", "에깅(에기 루어) 낚시도 동일 적용"],
    penaltyNote: "금어기 위반 시 300만원 이하 과태료",
    legalRef: "수산자원관리법 시행규칙 별표30",
    category: "saltwater",
...
export function searchRegulations(query: string): FishRegulation[] {
  ...
  return FISH_REGULATION_DB.filter(
    (r) =>
...
export function isCatchLegal(
  ...
  if (reg.closedSeason) {
    ...
    if (current >= start && current <= end) {
      violations.push(
        `현재 금어기 기간 (${reg.closedSeason.start}~${reg.closedSeason.end})`,
      );
    }
  }
  ...
  if (reg.minSizeCm && sizeCm !== null && sizeCm < reg.minSizeCm) {
    violations.push(
      `포획금지 체장 미달 (${sizeCm}cm < 최소 ${reg.minSizeCm}cm)`,
    );
  }
