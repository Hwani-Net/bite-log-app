...
const SPECIES_DB: SpeciesInfo[] = [
  {
    name: "감성돔",
    image:
      "https://search.pstatic.net/common/?src=http%3A%2F%2Fimgnews.naver.net%2Fimage%2F5119%2F2017%2F05%2F12%2F0000593409_001_20170512060327273.png&type=l340_165",
  ...
  },
  {
    ...
    optimalTemp: [10, 25],
    difficulty: "beginner",
    techniques: ["외줄낚시", "루어낚시"],
  },
  {
    name: "볼락",
    image:
      "https://search.pstatic.net/common/?src=http%3A%2F%2Fblogfiles.naver.net%2FMjAyNTExMjdfMzEg%2FMDAxNzY0MjMwMzY2ODI5.D02DZ_0aJMFTE4xNn1bJEgN-muC5HLCXx99OUNQN7JEg.BSx5RD94P2d9lKw5Zudg1kYsPMVQGxQgcpJC_jTX00Yg.JPEG%2F20251126_015326.jpg&type=a340",
    seasons: [11, 12, 1, 2, 3],
    optimalTemp: [5, 15],
    difficulty: "beginner",
    techniques: ["메바링", "볼락루어"],
  },
  {
    name: "농어",
    image:
      "https://search.pstatic.net/sunny/?src=https%3A%2F%2Fmblogthumb-phinf.pstatic.net%2FMjAyNDEyMzFfMjI5%2FMDAxNzM1NjMzNTA0NDkw.CY892IopGCGUR7f-lbrSjVhj-Q8vZ7c3r0wO7jbveEog.oG5NISyB3Y5-CB1QmjxgN7tyHxkb19Bw14ZkloU7Pq8g.JPEG%2FIMG_6904.jpg%3Ftype%3Dw800&type=a340",
    seasons: [5, 6, 7, 8, 9],
    optimalTemp: [15, 28],
    difficulty: "advanced",
    techniques: ["루어낚시", "미노우"],
  ...
  {
    name: "참돔",
    seasons: [4, 5, 6, 9, 10, 11],
    optimalTemp: [14, 24],
    difficulty: "advanced",
    techniques: ["타이라바", "참돔찌낚시"],
  },
  {
    name: "노래미",
    image:
      "https://search.pstatic.net/sunny/?src=https%3A%2F%2Fwww.innak.kr%2Fdata%2Ffile%2Fpdspictorial%2F50_20061122_134854_1.jpg&type=a340",
...
const GEAR_DB: GearItem[] = [
  ...
  {
    id: "rod-1",
    name: "다이와 갯바위 1.5-530",
    category: "rod",
    price: 380000,
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCIXdRg77TJro2W2OLgABkOBYqjxjabsY-0FZXiNUNeqrak_xRCxG9qPXnwQHioc8dCOavNF_p2Ec8t6NOWQ7eb53DimDUmJT_r2v05CrSxDzah5PmozHnrGBH8wY0bPTOGdALgX3s3FOWsWR80qrcNYu1b0ohGdPHMGb3rXRxU-RTpR3V3KJtj1KNHkZq5E1N_P0OmC0Gq-cDOATyouqfL_eAdlBDWQR3zAPUdE8sqt-g-huu5-dMPDWmn8R-xyxz0t9O25dLLjUri",
    species: ["감성돔", "벵에돔", "참돔"],
    trending: true,
  },
  {
    id: "rod-2",
    name: "메이저크래프트 루어로드 ML",
    category: "rod",
    price: 165000,
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCIXdRg77TJro2W2OLgABkOBYqjxjabsY-0FZXiNUNeqrak_xRCxG9qPXnwQHioc8dCOavNF_p2Ec8t6NOWQ7eb53DimDUmJT_r2v05CrSxDzah5PmozHnrGBH8wY0bPTOGdALgX3s3FOWsWR80qrcNYu1b0ohGdPHMGb3rXRxU-RTpR3V3KJtj1KNHkZq5E1N_P0OmC0Gq-cDOATyouqfL_eAdlBDWQR3zAPUdE8sqt-g-huu5-dMPDWmn8R-xyxz0t9O25dLLjUri",
  ...
  // Lures
  {
    ...
    price: 15000,
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAH_wFIpljKpsc3egIpdFKpTfDWIWY1ZIXK-L0goovoB5v9DqyYgNqWeBnGK0IvxqrlqmOgo3NcrhCzjaAa0DkTuVPAqGglwdV2005U1HzE6um9r7LFlKsSIKpTOycsr1JuXKW4l9mgpMkhExy5ZvU1GuS1q_v97GP1WTL7pKJIbFEARoceZNFdJI7lW6do5z0-lNvMhTT5H7kENNJEbL1b0nqwizE-r4k6NyDNZQ7b5xx7wh5rGbxnjLeQyWd9OnK5EJwnOJYMuQTt",
    species: ["오징어"],
    trending: true,
  },
  {
...
function scoreSpecies(
  ...
  if (weather?.tempC) {
    if (
      weather.tempC >= species.optimalTemp[0] &&
      weather.tempC <= species.optimalTemp[1]
    ) {
      score += 25;
    } else {
      const diff = Math.min(
...
function generateTips(
  ...
  if (weather) {
    if ((weather.windSpeed ?? 0) > 8) {
      tips.push("바람이 강합니다. 무거운 봉돌과 안전장비를 준비하세요.");
    }
    if (weather.tempC < 5) {
      tips.push("체감온도가 낮습니다. 방한복 필수!");
    }
    if (weather.tempC > 28) {
      tips.push("무더위 주의! 자외선 차단과 수분 보충을 챙기세요.");
    }
    if (
      weather.conditionKo.includes("비") ||
      weather.conditionKo.includes("소나기")
    ) {
      tips.push("비 소식이 있습니다. 우비와 방수가방을 챙기세요.");
    }
  }
...
export function generateRecommendation(
  ...
  const scoredSpots = FISHING_SPOTS.map((spot) => ({
    spot,
    score: scoreSpot(spot, month, weather, userLat, userLng),
  ...
  const bestSpecies =
    scoredSpecies[0]?.species ??
    SPECIES_DB.filter((sp) => sp.seasons.includes(month)).sort(
      (a, b) =>
        scoreSpecies(b, month, weather) - scoreSpecies(a, month, weather),
    )[0] ??
    SPECIES_DB[0];
  ...
  const matchedGear = GEAR_DB.filter((g) =>
    g.species.some((s) => s === bestSpecies.name),
  )
  ...
  return {
    ...
    reasoning,
    secretSpot: isPro
      ? SECRET_FISHING_SPOTS[
          Math.floor(Math.random() * SECRET_FISHING_SPOTS.length)
        ]
      : undefined,
  };
}
...
function buildReasoning(
  ...
  if (weather) {
    ...
    if (
      species.optimalTemp[0] <= (weather.tempC ?? 15) &&
      (weather.tempC ?? 15) <= species.optimalTemp[1]
    ) {
...
export function getTopSpots(
  ...
  return FISHING_SPOTS.map((spot) => {
    ...
    if (bestSpeciesNow.length > 0)
      parts.push(`${bestSpeciesNow.join("·")} 가능`);
...
export function generateCatchReport(
  ...
  const topSpecies = [...speciesMap.entries()]
    ...
    .map(([name, count]) => ({
      name,
      count,
      pct: totalFish > 0 ? Math.round((count / totalFish) * 100) : 0,
    }));
  ...
  const biggestFish = biggest
    ? {
        species: biggest.species || "미확인",
        sizeCm: biggest.sizeCm!,
        date: biggest.date,
      }
    : null;
  ...
  const avgPerTrip =
    totalTrips > 0 ? Math.round((totalFish / totalTrips) * 10) / 10 : 0;

  // Trend vs previous month
  const prevFish = prevRecords.reduce((sum, r) => sum + (r.quantity ?? 1), 0);
  const trendVsPrev =
    prevFish > 0 ? Math.round(((totalFish - prevFish) / prevFish) * 100) : null;
  ...
  if (totalTrips === 0) {
    ...
    monthSummary =
      `${monthLabel} ${totalTrips}회 출조, 총 ${totalFish}마리를 잡았습니다.` +
