...
function assignDefaultThumbnail(
...
function isFishingRelevant(text: string): boolean {
  ...
  const isShoppingOrSpam = SHOPPING_KEYWORDS.some((kw) =>
...
export async function fetchNaverNews(
  ...
  try {
    const [blogRes, newsRes] = await Promise.allSettled([
      fetch(
        `/api/naver?type=blog&query=${encodeURIComponent(query)}&display=${display}&sort=${sort}`,
      ),
      fetch(
        `/api/naver?type=news&query=${encodeURIComponent(query)}&display=${display}&sort=${sort}`,
      ),
    ]);
    ...
    const relevant = items.filter((it) =>
      isFishingRelevant(it.title + " " + it.description),
    );
    relevant.sort(
      (a, b) =>
...
export async function fetchAllFishingNews(
  ...
  const regionQuery =
    regionFilter !== "all" ? REGION_KEYWORDS[regionFilter]?.[0] || "" : "";
  ...
  if (sourceFilter === "all" || sourceFilter === "youtube") {
    ...
    const ytQuery =
      `실시간 ${regionQuery} 낚시 조과`.trim() || "실시간 낚시 조황";
  ...
  if (regionFilter !== "all") {
    allItems = allItems.filter(
      (item) => !item.region || item.region === regionFilter,
    );
  }
  ...
  return [
    {
      ...
      description:
        "통영 욕지도 일대에서 감성돔 40cm급이 연일 올라오고 있습니다. 물때와 수온이 딱 맞아떨어지면서...",
    ...
    },
    {
      ...
      description:
        "태안 만리포 방파제에서 볼락 20cm급이 연이어 히트! 지그헤드 1.5g + 웜 조합이 특효...",
    ...
    },
    {
      ...
      description:
        "제주 모슬포 앞바다에서 올해 첫 방어 시즌이 개막했습니다. 지깅으로 80cm급이...",
    ...
    },
    {
      ...
      description:
        "속초항 방파제에서 밤 8시부터 새벽 2시까지 한 박스 가득 채웠습니다. 2.5호 에기가...",
    ...
    },
    {
      ...
      description:
        "여수 일대 연안에서 광어 40~50cm급이 잡히기 시작했습니다. 다운샷 리그 추천...",
