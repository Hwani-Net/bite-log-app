...
function estimateTideNum(
  tideData: Awaited<ReturnType<typeof fetchTideData>>,
): number {
...
export async function generateTripBriefing(
  ...
  const tideInfo = tideResult
    ? tideResult.tides
        .map((t) => `${t.type === "High" ? "만조" : "간조"} ${t.time}`)
        .join(" / ")
    : "조수 정보 없음";
  ...
  const insights =
    communityResult.status === "fulfilled"
      ...
      : { articles: [], summary: "없음" };
  const communityInsights: CommunityInsight[] = insights.articles
    .slice(0, 5)
    .map((a) => ({
 ...
 */
export function getBriefingAlertTime(
  tripDate: string,
  alertHour: number = 14,
): Date {
