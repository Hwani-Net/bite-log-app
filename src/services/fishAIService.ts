...
const MOCK_RESULTS: FishAIResult[] = [
  { species: 'Red Sea Bream', koreanName: '참돔', confidence: 92, description: '한국 연안에서 흔히 볼 수 있는 고급 어종입니다. 체장 30~50cm가 일반적이며, 봄~가을 시즌이 최적입니다.' },
  { species: 'Black Rockfish', koreanName: '우럭', confidence: 88, description: '바위 지대에 서식하는 대표적인 선상 낚시 대상어입니다. 연중 낚이며, 특히 겨울철 조과가 좋습니다.' },
  { species: 'Flatfish', koreanName: '광어', confidence: 85, description: '모래 바닥에 서식하는 저서어류로, 루어 낚시의 인기 대상어입니다.' },
  { species: 'Korean Rockfish', koreanName: '볼락', confidence: 90, description: '야간 낚시의 대표 어종으로, 라이트게임에 적합합니다.' },
  { species: 'Webfoot Octopus', koreanName: '주꾸미', confidence: 95, description: '가을 시즌의 대표 대상종으로, 에기 낚시로 많이 잡힙니다.' },
];
...
export async function identifyFish(photoBase64: string): Promise<FishAIResult | null> {
  ...
  if (!apiKey) {
    console.warn('Gemini API key not configured. Returning mock AI result for development.');
    // Return random mock result after simulated delay
