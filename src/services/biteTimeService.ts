...
function getWindScore(windSpeed?: number): {
  ...
  if (windSpeed === undefined)
    return { score: 8, status: "neutral", description: "바람 정보 없음" };
...
function getSSTScore(
  ...
  if (temp === undefined || temp === 0)
    return { score: 8, status: "neutral", description: "수온 정보 없음" };
...
function getTideScore(
  ...
  const scoreMap: Record<
    string,
    { score: number; status: "positive" | "neutral" | "negative" }
  > = {
...
function getPressureScore(pressureMsl?: number): {
  score: number;
  status: "positive" | "neutral" | "negative";
  description: string;
} {
  if (pressureMsl === undefined)
    return { score: 7, status: "neutral", description: "기압 정보 없음" };
...
function getWaveScore(waveHeight?: number): {
  ...
  if (waveHeight === undefined || isNaN(waveHeight))
    return { score: 5, status: "neutral", description: "파고 정보 없음" };
...
function getLunarScore(lunar: LunarInfo): {
  score: number;
  status: "positive" | "neutral" | "negative";
  description: string;
} {
...
function getGrade(score: number): {
  grade: BiteTimePrediction["grade"];
  gradeLabel: string;
  gradeEmoji: string;
} {
  if (score >= 75)
  ...
  if (score >= 55)
    return { grade: "good", gradeLabel: "좋은 조건", gradeEmoji: "🔵" };
  if (score >= 35)
    return { grade: "fair", gradeLabel: "보통", gradeEmoji: "🟡" };
...
export function calculateBiteTime(
  ...
  const reliableSST =
    marine?.seaSurfaceTemp && marine.seaSurfaceTemp > 0
 ...
 */
export function getPeakFishingWindows(
  tideData: TideData | null,
): TimelineSlot[] {
...
function getEnvironmentScore(hour: number): { score: number; tag: string } {
  ...
  if (hour >= 8 && hour <= 10) return { score: 55, tag: "오전" };
  if (hour >= 15 && hour <= 16) return { score: 55, tag: "오후" };
  if (hour >= 11 && hour <= 14) return { score: 40, tag: "한낮" };
  if (hour >= 21 || hour <= 3) return { score: 30, tag: "야간" };
  return { score: 45, tag: "🕐 전환 시간대" };
}

// 조석점수: 0~100
function getHourlyTideScore(
  hour: number,
  tideData: TideData | null,
): { score: number; tag: string; isPeak: boolean } {
...
export function getSpeciesPeakWindows(
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    const score = Math.round(
      fishActivity * 0.5 + tide.score * 0.3 + env.score * 0.2,
    );
