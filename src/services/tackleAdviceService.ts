...
export function getTideLabel(tideNum: number): string {
  ...
  const phaseLabel = {
    neap: "조금 (약한 조류)",
    moderate: "보통 조류",
    spring: "사리 (강한 조류)" }[phase];
  return `${tideNum}물 — ${phaseLabel}`;
}

// ─── 어종별 채비 데이터베이스 ──────────────────────────────────

const TACKLE_DB: Record<
  string,
  {
...
> = {
  주꾸미: {
    sinker: {
      neap: {
        tungsten: "4~8호",
        lead: "8~12호",
        note: "조류 약함, 가벼운 봉돌로 바닥 탐색" },
      moderate: {
        tungsten: "6~14호",
        lead: "12~18호",
        note: "보통 조류, 수심 20~30m 기준" },
      spring: {
        tungsten: "10~20호",
        lead: "16~28호",
        note: "조류 강함, 무거운 봉돌 필수" } },
    line: {
      mainLine: "합사 0.6~1호",
      leader: "카본 1.5~2호",
      length: "최소 100m" },
  ...
  방어: {
    sinker: {
      neap: {
        tungsten: "100~150g",
        note: "수심 50~80m, 조류 약할 때 가벼운 지그" },
      moderate: { tungsten: "150~250g", note: "수심 60~100m, 슬로우지깅" },
      spring: {
        tungsten: "200~300g",
        note: "조류 강할 때 무거운 지그로 빠르게 침강" } },
    line: {
      mainLine: "합사 PE2~4호",
      leader: "카본 10~16호",
      length: "최소 300m" },
...
export function getWeatherChecklist(params: {
  ...
  if (temp <= 5) {
    ...
    items.push({
      icon: "Hand",
      item: "방한 낚시 장갑",
      reason: "손이 얼면 라인 컨트롤 불가",
      priority: "essential",
      coupangQuery: "낚시 방한 장갑" });
  } else if (temp <= 12) {
    items.push({
      icon: "Shirt",
      item: "방한 자켓",
      reason: `기온 ${temp}℃, 바람 체감 추움`,
      priority: "essential",
      coupangQuery: "방수 낚시 자켓" });
    items.push({
      icon: "Flame",
      item: "핫팩 5개",
      reason: "예비 보온",
  ...
  if (uv >= 8) {
    items.push({
      icon: "Glasses",
      item: "선글라스 (편광)",
      reason: `자외선 지수 ${uv} — 매우 강함`,
      priority: "essential",
      coupangQuery: "편광 낚시 선글라스" });
    items.push({
      icon: "Droplets",
      item: "자외선 차단제 SPF50+",
      reason: "장시간 야외, 피부 화상 위험",
      priority: "essential",
      coupangQuery: "선크림 SPF50" });
  } else if (uv >= 5) {
    items.push({
      icon: "Glasses",
      item: "선글라스",
  ...
  if (windSpeed >= 8) {
    items.push({
      icon: "AlertTriangle",
      item: "배멀미약 (전날 복용)",
      reason: `풍속 ${windSpeed}m/s — 선상 출렁임 심함`,
      priority: "essential",
      coupangQuery: "멀미약" });
    items.push({
      icon: "BandageIcon",
  ...
  const essentials: ChecklistItem[] = [
      ...
      priority: "essential" },
    {
      icon: "Shield",
      item: "구명조끼 (자동팽창식)",
      reason: "안전 필수 — 선상낚시 착용 의무",
      priority: "essential",
      coupangQuery: "자동팽창식 구명조끼" },
  ];
