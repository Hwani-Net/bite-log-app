...
export interface MarineData {
  ...
  hourlySst?: number[]; // °C per hour
  hourlyWaveHeight?: number[]; // m per hour
  hourlyTimes?: string[]; // ISO datetime strings
}
...
const COASTAL_POINTS: { name: string; lat: number; lng: number }[] = [
  { name: "인천 앞바다", lat: 37.45, lng: 126.4 },
  { name: "태안 서해", lat: 36.75, lng: 126.1 },
  { name: "군산 앞바다", lat: 35.95, lng: 126.55 },
  { name: "목포 앞바다", lat: 34.75, lng: 126.3 },
  { name: "여수·통영", lat: 34.6, lng: 128.0 },
  { name: "부산 앞바다", lat: 35.05, lng: 129.1 },
  { name: "울산 앞바다", lat: 35.5, lng: 129.45 },
  { name: "포항 앞바다", lat: 36.05, lng: 129.55 },
  { name: "속초 동해", lat: 38.18, lng: 128.6 },
...
export async function fetchMarineData(
  ...
  try {
    ...
    if (
      result &&
      (result.waveHeight === undefined || isNaN(result.waveHeight))
    ) {
      const coast = findNearestCoast(lat, lng);
      console.info(
        `[Marine] Inland detected (no wave data). Snapping to ${coast.name} (${coast.lat}, ${coast.lng})`,
      );
...
async function fetchMarineDataRaw(
  lat: number,
  lng: number,
): Promise<MarineData | null> {
  const url = `https://marine-api.open-meteo.com/v1/marine?latitude=${lat}&longitude=${lng}&current=sea_surface_temperature,wave_height,wave_period,ocean_current_velocity,ocean_current_direction&hourly=sea_surface_temperature,wave_height&forecast_days=3&timezone=auto`;
  ...
  const hourlySst = hourly?.sea_surface_temperature as number[] | undefined;
  const hourlyWaveHeight = hourly?.wave_height as number[] | undefined;
  ...
  return {
    ...
    waveHeight:
      waveH != null
        ? Math.round(waveH * 100) / 100
        : (undefined as unknown as number),
    wavePeriod:
      waveP != null
        ? Math.round(waveP * 10) / 10
        : (undefined as unknown as number),
    currentVelocity:
      current.ocean_current_velocity != null
        ? Math.round(current.ocean_current_velocity * 100) / 100
        : undefined,
    currentDirection:
      current.ocean_current_direction != null
        ? Math.round(current.ocean_current_direction)
        : undefined,
    hourlySst: hourlySst ?? undefined,
    hourlyWaveHeight: hourlyWaveHeight ?? undefined,
